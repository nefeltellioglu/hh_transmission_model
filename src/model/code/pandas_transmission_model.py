...
def run_SEIR_model(p: Params):
    ...
    for run in range(p.no_runs):
        ...
        household = pd.DataFrame({
            "id": range(p.hh_size),
            "state": ["Susceptible"] * p.hh_size,
            "s_time_exposed": [0.0] * p.hh_size,
            "s_time_infectious": [0.0] * p.hh_size,
            "s_time_recovery": [0.0] * p.hh_size,
            "exposed_from": [-1] * p.hh_size,
            "hh_size": p.hh_size,
            "hh_id": run,
            "run_no": run,
        })
        ...
        if p.record_transmission:
            cur_records = pd.DataFrame()
        
        # Infect one individual in the household (seed infection)
        seed_infection_index = 0  # rng.randint(0, p.hh_size - 1)
